...
date: 2025-10-08
version: v2.0
permalink: claudedocs/企微聊天-api-设计
---
...
**基础路径**: `/wecom/staff-customer-relation`

**设计说明**: 基于 `t_wecom_staff_customer_relation` 表，管理员工与客户的 N:N 关系，支持员工级别的备注、标签等数据

### 1. 查询员工的客户列表

**接口**: `POST /wecom/staff-customer-relation/by-staff`
...
```java
@Data
public class WecomStaffCustomerRelationByStaffForm extends PageParam {
    @NotBlank(message = "员工ID不能为空")
    private String wecomStaffUserId;

    private Integer relationStatus;  // 可选, 0-已删除 1-正常
    private String keyword;          // 客户名称搜索
}
```

**响应**: `ResponseDTO<PageResult<WecomStaffCustomerRelationVO>>`

```java
@Data
public class WecomStaffCustomerRelationVO {
    ...
    // 员工级别数据
    private String staffRemark;         // 员工备注名 (优先展示)
    private String staffDescription;    // 员工描述
    private Integer addWay;             // 添加来源
    private LocalDateTime addTime;      // 添加时间
    private String state;               // 来源渠道
    private String remarkCorpName;      // 备注企业名称
    private List<String> remarkMobiles; // 备注手机号
    private List<CustomerTagVO> tags;   // 客户标签
...
public class CustomerTagVO {
    ...
    private Integer type;  // 1-企业标签 2-个人标签 3-规则组标签
}
```
...
**接口**: `POST /wecom/staff-customer-relation/by-customer`
...
```java
@Data
public class WecomStaffCustomerRelationByCustomerForm extends PageParam {
    @NotBlank(message = "客户ID不能为空")
    private String wecomExternalUserId;
...
**接口**: `GET /wecom/staff-customer-relation/detail/{relationId}`
...
**响应**: `ResponseDTO<WecomStaffCustomerRelationDetailVO>`

```java
@Data
public class WecomStaffCustomerRelationDetailVO extends WecomStaffCustomerRelationVO {
    // 员工完整信息
    private WecomStaffVO staffInfo;
    // 客户完整信息
    private WecomCustomerVO customerInfo;
}
```
...
**接口**: `POST /wecom/staff-customer-relation/update`
...
```java
@Data
public class WecomStaffCustomerRelationUpdateForm {
    @NotNull
    private Long relationId;

    private String staffRemark;         // 员工备注名
    private String staffDescription;    // 员工描述
    private String remarkCorpName;      // 备注企业名称
    private List<String> remarkMobiles; // 备注手机号
    private List<CustomerTagVO> tags;   // 客户标签
...
### v2.0 (2025-10-08) - 同步数据库设计 v2.3

**核心变更**:

1. ✅ **消息会话解耦架构**
   - 新增 `POST /wecom/message/by-participant` - 查询某人收到的所有消息（极速查询）
   - `WecomMessageVO` 移除 `conversationId` 字段
   - 新增 `WecomMessageDetailVO.relatedConversations` - 消息关联的会话列表
   - SQL 实现基于 `t_wecom_message_conversation` 关联表

2. ✅ **新增员工客户关系管理模块**
   - 新增 `POST /wecom/staff-customer-relation/by-staff` - 查询员工的客户列表
   - 新增 `POST /wecom/staff-customer-relation/by-customer` - 查询客户的员工列表
   - 新增 `GET /wecom/staff-customer-relation/detail/{relationId}` - 查询关系详情
   - 新增 `POST /wecom/staff-customer-relation/update` - 更新备注和标签
   - 支持员工级别的备注、标签、添加来源等数据

3. ✅ **字段规范化调整**
   - 所有企微原始 ID 字段统一 `wecom_` 前缀:
     - `userId` → `wecomUserId`
     - `externalUserid` → `wecomExternalUserId`
     - `chatId` → `wecomChatId`
     - `ownerId` → `wecomOwnerId`
   - 所有标志字段统一 `_flag` 后缀:
     - `hasConversation` → `hasConversationFlag`
     - `enableChatArchive` → `enableChatArchiveFlag`

4. ✅ **移除功能**
   - 删除会话收藏功能接口:
     - ❌ `POST /wecom/conversation/favorite`
     - ❌ `POST /wecom/conversation/unfavorite`
   - 删除 `WecomConversationVO` 的收藏相关字段:
     - ❌ `isFavorite`
     - ❌ `favoriteReason`
     - ❌ `favoriteTime`
   - 删除 `WecomConversationStatsVO.favoriteCount` 统计字段

5. ✅ **接口数量调整**
   - 企业配置管理: 8 → 9 个接口
   - 消息管理: 8 → 10 个接口（新增2个，移除0个）
   - 会话管理: 10 → 8 个接口（移除收藏功能）
   - 员工客户关系: 0 → 4 个接口（新增模块）
   - 总计: 58 → 68 个接口

**性能优化**:
- 查询某人的消息性能提升 3-5 倍（基于消息会话关联表单表索引扫描）
- 避免消息查询的 3 表 JOIN，改用 2 表 JOIN

**数据库适配**:
- 基于数据库设计 v2.3（消息会话解耦架构）
- 新增 `t_wecom_message_conversation` 表支持
- 新增 `t_wecom_staff_customer_relation` 表支持

---

### v1.0 (2025-10-08) - 初版发布
